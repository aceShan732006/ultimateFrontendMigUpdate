...
### Purpose
Executing the migration plan by applying code and configuration changes for **one version jump at a time**, but only execute the v17 -> v18 migration path.
...
- **Workflow Enforcement:** Strictly execute the v17->v18
...
- When the command `implement the migration plan` is issued, the Implementation Agent MUST perform the entire v17 → v18 migration autonomously and end-to-end, including assessment and planning steps if they have not yet run.
...
- The agent MUST run the following sequence without any interactive prompts or manual confirmations:
 ...
 3. `ng test -- --watch=false` (run targeted specs when full-suite is impractical)
  4. If gates pass: `git status`, `git add -A`, `git commit -m "chore(migration): complete Angular v18"`, `git push origin HEAD`.
