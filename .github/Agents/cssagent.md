...
- **Asset Path Correction:** Resolve relative asset paths (backgrounds, fonts) that break during the v17→v18 builder transition.
- **Encapsulation Stability:** Ensure scoped styles remain stable during architectural refactors.

Note: The above tasks are prioritized by risk for the v17→v18 migration. Avoid large-scale visual rewrites unless they directly address a build or runtime compatibility issue for v18.
...
2. **Phase Fit:** Coordinate with the planning agent to schedule style updates during builder transitions (v17→v18) and complex component refactors.
3. **Execution:**
