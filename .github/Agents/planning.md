...
- When `implement the migration plan` is issued, the Planning Agent MUST execute automatically to produce or refresh `plan/migration_v17_to_v18.md` and any required per-component checklists. No user confirmation is allowed.
...
- The plan must include a mandatory checkpoint(never use tags n branches for checkpoints; only check commits for checkpoints) sequence that the Implementation Agent will follow: `npm install`, `ng build`, `ng test -- --watch=false`, `git status`, `git add -A`, `git commit -m "chore(migration): complete Angular v18"`, `git push`.
...
### Outputs
  ...
  - Detailed v17 → v18 transition sequence.
  - Manual step cross-references.

### MUST INCLUDE: OUTPUT
- **Plan File (file):** plan/migration_v17_to_v18.md
